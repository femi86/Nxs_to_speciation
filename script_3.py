...
LCF_energies = ['12630','12658','12671','12677','12686','12726','12755']
...
att_Tl = 10.02#tx_Tl * att_mass_Tl
att_Mn = 1#tx_Mn * att_mass_Mn
att_As = 1#tx_As * att_mass_As
...
nor_vals_full = np.zeros((max_en_fit-en_lo,len(ref_list)))
...
for index,reference in enumerate(ref_list):
    ...
    i = 0
    for enn in range(en_lo,max_en_fit):
        nor_vals_full[i,index] = ref[np.where(np.around(ref[:,0],decimals=0)==enn)[0][0],1]
        i+=1
...
text_lcf_1 = ['LCF_red_comp','LCF_ox_comp','LCF_1comp','LCF_residual','LCF_red_value']
...
LCF_full = {os.path.basename(filename):{} for filename in folder}
norm_like_script = {os.path.basename(filename):[] for filename in folder}

for indx,filename in enumerate(folder):
    tmparray = np.loadtxt(filename)
    index = os.path.basename(filename)
    lo_en = np.where(tmparray[:,0]==en_lo)[0][0]
    hi_en = np.where(tmparray[:,0]==en_hi)[0][0]
    norm_like_script[index] = (tmparray[:,1]-tmparray[lo_en,1])/(tmparray[hi_en,1]-tmparray[lo_en,1])
    norm_like_script[index] = norm_like_script[index][np.where(tmparray[:,0]==en_lo)[0][0]:np.where(tmparray[:,0]==max_en_fit)[0][0]]
    ...
    data = [raw_pxanes,norm_like_script]
    normalized = [nor_vals,nor_vals_full]
    outputs = [LCF,LCF_full]
    for x in range(0,2):
        pix_1_comp, pix_1_res, pix_1_val  = pixelLCF(data[x][index],normalized[x],ref_list)   
        pix_2_comp, pix_2_res, pix_2_val = pixel_2compLCF(data[x][index],normalized[x],couples)
        ...
        out5 = sel
        outputs[x][index] = [out1[0],out1[1],out1_5, out2, out3[0], out3[1], out4[0],out4[1], out5, Tl_v, Tl_v/Mn_v, Tl_v/As_v]   
...
lcf_full = pd.DataFrame.from_dict(LCF,orient='index',columns=text_lcf)

writer = pd.ExcelWriter(os.path.join(path,'output.xlsx'),engine='xlsxwriter')
raw.to_excel(writer,sheet_name='raw_data')
lcf_out.to_excel(writer,sheet_name='LCF_out_simple')
lcf_full.to_excel(writer,sheet_name = 'LCF_out_full')
writer.save()
